...
class QuadrupedWalker:
    ...
    def __init__(self, leg_dims: LegDimensions = None,
        ...
        self.neutral_feet = {
            ...
            LegID.RL: (rear_backward,  foot_lateral, -h),
            LegID.RR: (rear_backward, -foot_lateral, -h),
        }
        ...
        # Auto-recording: logs every tick while moving, saves on stop/quit
        self._auto_log = []
    ...
    def save_auto_log(self):
        ...
        # Summary: per-leg angle ranges and goal-vs-actual error
        prefix_map = {LegID.FL: 'FL', LegID.FR: 'FR',
                      LegID.RL: 'RL', LegID.RR: 'RR'}
        print(f"\n  {'Leg':4s} {'Joint':6s}  {'Angle min':>9s} {'max':>6s} {'range':>6s}  "
              f"{'Raw min':>7s} {'max':>5s}  {'Avg err':>7s} {'Max err':>7s}")
        print(f"  {'-'*72}")
        for leg_id in LegID:
            prefix = prefix_map[leg_id]
            for jname in ['shldr', 'femur', 'tibia']:
                deg_key = f'{prefix}_{jname}_deg'
                raw_key = f'{prefix}_{jname}_raw'
                goal_key = f'{prefix}_{jname}_raw'
                actual_key = f'{prefix}_{jname}_actual'
                degs = [r[deg_key] for r in self._auto_log if deg_key in r]
                raws = [r[raw_key] for r in self._auto_log if raw_key in r]
                errors = []
                for rec in self._auto_log:
                    g = rec.get(goal_key, -1)
                    a = rec.get(actual_key, -1)
                    if a != -1 and g != -1:
                        errors.append(abs(g - a))
                if degs and raws:
                    avg_e = f"{sum(errors)/len(errors):.1f}" if errors else "N/A"
                    max_e = f"{max(errors):.0f}" if errors else "N/A"
                    print(f"  {prefix:4s} {jname:6s}  {min(degs):+9.1f} {max(degs):+6.1f} "
                          f"{max(degs)-min(degs):6.1f}  {min(raws):5d}   {max(raws):5d}  "
                          f"{avg_e:>7s} {max_e:>7s}")
        print()
    ...
    def _control_loop(self):
        ...
        while self._running:
            ...
            # Auto-record every tick when moving (comprehensive data for gait tuning)
            if is_moving and self._auto_log_start > 0:
                record = {
                    'time': round(time.monotonic() - self._auto_log_start, 3),
                ...
                prefix_map = {LegID.FL: 'FL', LegID.FR: 'FR',
                              LegID.RL: 'RL', LegID.RR: 'RR'}
                # Read actual servo positions every 5th tick (serial reads are slow)
                do_actual_read = (len(self._auto_log) % 5 == 0)
                for leg_id in LegID:
                    prefix = prefix_map[leg_id]
                    ...
                    if not ld:
                        continue
                    # Trajectory offsets
                    record[f'{prefix}_phase'] = round(ld['phase'], 4)
                    record[f'{prefix}_state'] = ld['state']
                    record[f'{prefix}_dx'] = round(ld['dx'], 2)
                    record[f'{prefix}_dy'] = round(ld['dy'], 2)
                    record[f'{prefix}_dz'] = round(ld['dz'], 2)
                    # Absolute foot position
                    record[f'{prefix}_foot_x'] = round(ld['foot_x'], 2)
                    record[f'{prefix}_foot_y'] = round(ld['foot_y'], 2)
                    record[f'{prefix}_foot_z'] = round(ld['foot_z'], 2)
                    # IK angles and servo raw values
                    joint_names = ['shldr', 'femur', 'tibia']
                    for i, cfg in enumerate(LEG_SERVO_CONFIG[leg_id]):
                        jname = joint_names[i]
                        record[f'{prefix}_{jname}_deg'] = round(ld['angles'][i], 2)
                        record[f'{prefix}_{jname}_raw'] = positions.get(cfg.servo_id, -1)
                        # Actual servo position (sub-sampled to avoid bus congestion)
                        if do_actual_read:
