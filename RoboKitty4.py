...
Changes:
    v1.9 - WEIGHT TRANSFER gait: switched default to WALK/creep gait
           (duty_factor=0.75, only 1 foot up at a time). Added body
           CoM shift toward support triangle before each swing. Ground
           press-down (-5mm) at start of stance for grip. Reduced
           step_length 100->70mm to prevent bounce/slip. Walk phase
           offsets: RL=0.0,FR=0.25,FL=0.5,RR=0.75. Cycle time 2.4s.
...
class FootTrajectory:
    ...
    def compute(self, phase: float, speed: float = 1.0) -> Tuple[float, float, float]:
        ...
        if phase < duty:
            ...
            # Ground press: push foot down slightly at start of stance
            # for better grip. Peaks at ~10% into stance then fades.
            if t < 0.15:
                dz = -5.0 * math.sin(math.pi * t / 0.15)  # press down 5mm
            else:
                dz = 0.0
        else:
