...
class GaitConfig:
    ...
    step_length: float = 60.0      # Forward travel per step mm (moderate stride)
    cycle_time: float = 3.0        # Full gait cycle seconds (slower swing)
    duty_factor: float = 0.5       # 0.5=trot, 0.75=walk (overridden per gait)
...
class FootTrajectory:
    ...
    def compute(self, phase: float, speed: float = 1.0) -> Tuple[float, float, float]:
        ...
        if phase < duty:
            # STANCE: foot on ground, cosine interpolation for smooth push
            t = phase / duty
            # Cosine gives smooth acceleration/deceleration (no jerk)
            dx = half_step * math.cos(math.pi * t) * speed
            dz = 0.0
        else:
            # SWING: foot lifts and repositions
            t = (phase - duty) / (1.0 - duty)
            # Cosine dx matches stance endpoints smoothly
            dx = -half_step * math.cos(math.pi * t) * speed
            # Gentle lift: quick up, hold, soft descent
            if t < 0.25:
                dz = self.cfg.step_height * math.sin(math.pi * t / 0.5)
            elif t < 0.65:
                dz = self.cfg.step_height
            else:
                descent_t = (t - 0.65) / 0.35
                dz = self.cfg.step_height * (1.0 - descent_t * descent_t)
...
class QuadrupedWalker:
    ...
    def __init__(self, leg_dims: LegDimensions = None,
        ...
        # Live recording buffer: list of dicts
        self._recording = False
        self._record_buf = []
        self._record_interval = 5  # Record every Nth control loop tick
        self._record_tick = 0
    ...
    def set_gait(self, gait: GaitType):
        with self._lock:
            ...
            if gait == GaitType.WALK:
                self.gait_cfg.duty_factor = 0.80  # 80% stance, 20% swing
                self.gait_cfg.cycle_time = 3.0    # Slow and smooth
            else:
                self.gait_cfg.duty_factor = 0.5
                self.gait_cfg.cycle_time = 2.0    # Trot/pace still moderate
    ...
    def start_recording(self):
        """Start recording live servo data."""
        self._record_buf.clear()
        self._record_tick = 0
        self._record_start_time = time.monotonic()
        self._recording = True
        print(f"  Recording started (every {self._record_interval} ticks)")

    def stop_recording(self):
        """Stop recording."""
        self._recording = False
        count = len(self._record_buf)
        print(f"  Recording stopped ({count} samples)")
    ...
    def save_recording(self, filename=None):
        ...
        if not self._record_buf:
            ...
            return
        if filename is None:
            filename = os.path.expanduser("~/live_walk.csv")
        
        # Build column headers from first record
        fieldnames = list(self._record_buf[0].keys())
        ...
        print(f"  Saved {len(self._record_buf)} samples to {filename}")
        
        # Print summary: goal vs actual error stats
        print("\n  Goal vs Actual error summary:")
        print(f"  {'Servo':<12s} {'Avg err':>8s} {'Max err':>8s} {'Missed':>7s}")
        print(f"  {'-'*40}")
        
        prefix_map = {LegID.FL: 'FL', LegID.FR: 'FR',
                      LegID.RL: 'RL', LegID.RR: 'RR'}
        for leg_id in LegID:
            prefix = prefix_map[leg_id]
            for jname in ['shldr', 'femur', 'tibia']:
                goal_key = f'{prefix}_{jname}_goal'
                actual_key = f'{prefix}_{jname}_actual'
                errors = []
                missed = 0
                for rec in self._record_buf:
                    g = rec.get(goal_key, -1)
                    a = rec.get(actual_key, -1)
                    if a == -1 or g == -1:
                        missed += 1
                    else:
                        errors.append(abs(g - a))
                if errors:
                    avg_err = sum(errors) / len(errors)
                    max_err = max(errors)
                    print(f"  {prefix} {jname:<6s}    {avg_err:6.1f}   {max_err:6.0f}   {missed:5d}")
                else:
                    print(f"  {prefix} {jname:<6s}    {'N/A':>6s}   {'N/A':>6s}   {missed:5d}")
    ...
    def _control_loop(self):
        ...
        while self._running:
            ...
            # Live recording: read actual servo positions periodically
            if self._recording and is_moving:
                self._record_tick += 1
                if self._record_tick >= self._record_interval:
                    self._record_tick = 0
                    record = {
                        'time': round(time.monotonic() - self._record_start_time, 3),
                    ...
                    for leg_id in LegID:
                        prefix = prefix_map[leg_id]
                        for i, cfg in enumerate(LEG_SERVO_CONFIG[leg_id]):
                            jname = ['shldr', 'femur', 'tibia'][i]
                            goal = positions.get(cfg.servo_id, -1)
                            record[f'{prefix}_{jname}_goal'] = goal
                    # Read actual positions (all 12 servos)
                    for leg_id in LegID:
                        prefix = prefix_map[leg_id]
                        for i, cfg in enumerate(LEG_SERVO_CONFIG[leg_id]):
                            jname = ['shldr', 'femur', 'tibia'][i]
                            actual, _ = self.servos.read_position(cfg.servo_id)
                            if actual is not None and 0 <= actual <= 1023:
                                record[f'{prefix}_{jname}_actual'] = actual
                            else:
                                record[f'{prefix}_{jname}_actual'] = -1
                    self._record_buf.append(record)
...
def run_keyboard_control(walker: QuadrupedWalker):
    ...
    print("  l       = log walk cycle to CSV (shows commanded positions)")
    print("  v       = toggle live recording (saves on quit)")
    print("  q       = quit")
    print("=" * 50 + "\n")
            ...
            while True:
                if select.select([sys.stdin], [], [], 0.1)[0]:
                    ...
                    if key == 'q':
                        ...
                        if recording:
                            walker.start_recording()
                        else:
                            walker.stop_recording()
                    elif key == 'w':
                        speed = -1.0  # Forward (toward rear servos)
                    elif key == 's':
                        speed = 1.0   # Backward
                    elif key == 'a':
                    ...
                    rec_status = f"  REC({len(walker._record_buf)})" if recording else ""
                    sys.stdout.write(f"\r  Speed: {speed:+.1f}  Turn: {turn:+.1f}  "
                                     f"Gait: {walker.gait_type.value}{rec_status}       ")
                    sys.stdout.flush()
        finally:
            termios.tcsetattr(sys.stdin, termios.TCSADRAIN, old_settings)
            # Save recording on exit if any data collected
            if walker._record_buf:
                print("\n")
                walker.stop_recording()
                walker.save_recording()
