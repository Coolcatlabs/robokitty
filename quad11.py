...
class QuadrupedWalker:
    ...
    def test_rear_legs(self):
        """Lift both rear legs (RL + RR) to confirm which legs the code thinks are 'rear'.
        Lifts femur forward by 30° from standing, holds 2s, returns.
        Front legs stay put. If the WRONG legs move, the leg IDs are swapped."""
        print("  === REAR LEG TEST ===")
        print("  The TWO legs that MOVE are what the code calls 'rear'.")
        print("  If the wrong pair moves, leg servo IDs are swapped.\n")

        # First stand all legs
        self.smooth_stand(1.0)

        # Get standing positions for all servos
        stand_pos = {}
        for leg_id in LegID:
            foot = self.neutral_feet[leg_id]
            angles = self.ik.solve(*foot)
            ...
            for i, cfg in enumerate(LEG_SERVO_CONFIG[leg_id]):
                stand_pos[cfg.servo_id] = angle_to_raw(angles[i], cfg)

        # Compute lifted rear positions: raise femur +30° from neutral
        lift_pos = dict(stand_pos)
        for leg_id in [LegID.RL, LegID.RR]:
            foot = self.neutral_feet[leg_id]
            angles = self.ik.solve(*foot)
            angles = (self.locked_coxa_angles[leg_id], angles[1], angles[2])
            # Lift femur forward by 30 degrees
            lifted_femur = angles[1] + 30.0
            lifted_angles = (angles[0], lifted_femur, angles[2])
            configs = LEG_SERVO_CONFIG[leg_id]
            for i, cfg in enumerate(configs):
                lift_pos[cfg.servo_id] = angle_to_raw(lifted_angles[i], cfg)
            print(f"  {leg_id.name}: servo IDs = "
                  f"shldr:{configs[0].servo_id} fem:{configs[1].servo_id} tib:{configs[2].servo_id}")

        # Slow speed for gentle motion
        for sid in self._all_servo_ids():
            self.servos.set_moving_speed(sid, 150)
            time.sleep(0.003)

        print("\n  Lifting REAR legs now...")
        self.servos.sync_write_positions(lift_pos)
        time.sleep(2.0)

        print("  Returning to stand...")
        self.servos.sync_write_positions(stand_pos)
        time.sleep(1.5)
        ...
        print("  Test complete. Did the BACK two legs move?")
...
def run_keyboard_control(walker: QuadrupedWalker):
    ...
    print("  z       = test rear legs (lifts RL+RR to confirm)")
