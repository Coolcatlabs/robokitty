...
Changes:
    v2.3 - BIGGER STEPS + ZERO COMPLIANCE: step_length 40->80mm,
           cycle_time 2.4->1.6s so each 20ms control cycle moves
           the femur by 3.6 servo units (was 0.5 units = below the
           AX-12A compliance dead zone = no torque applied).
           Set compliance margin=0 on all servos to eliminate the
           1-unit dead zone where no force is applied.
           step_height 18->25mm for reliable ground clearance.
           Combined with v2.2 direction fix (stance pushes toward
           physical front at -x).
