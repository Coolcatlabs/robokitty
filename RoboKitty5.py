...
Changes:
    v2.0 - GROUND GRIP: sustained -10mm ground press throughout stance
           phase (sine curve peaking at mid-stance) forces tibia to
           extend into ground for traction. Removed --simulate mode
           and SimulatedServoInterface to reduce code size.
    v1.9 - WEIGHT TRANSFER: default WALK/creep gait (duty=0.75, 1 foot
           up at a time). Body CoM shift toward support triangle.
           step_length 70mm. Walk phases: RL=0,FR=0.25,FL=0.5,RR=0.75.
    v1.8 - Large gait swing (100mm step), audit logging.
    v1.7 - Applied calibrated offsets from --calibrate.
    v1.0 - Initial RoboKitty3 release.
...
class GaitConfig:
    ...
    step_length: float = 70.0      # Forward travel per step mm (reduced from 100 for grip)
    cycle_time: float = 2.4        # Full gait cycle seconds (slower for creep gait)
...
class FootTrajectory:
    """
    Generates smooth foot paths with body weight shifting.

    Stance: foot pushes backward on ground with initial press-down
    Swing: foot lifts and swings forward
    Body shift: during walk gait, shifts CoM over support triangle
    """
    ...
    def compute(self, phase: float, speed: float = 1.0) -> Tuple[float, float, float]:
        ...
        if phase < duty:
            # STANCE: foot pushes backward on ground
            t = phase / duty
            dx = half_step * (1.0 - 2.0 * t) * speed

            # Sustained ground press: push foot DOWN throughout stance.
            # Sine curve peaks at mid-stance (-10mm) when foot is directly
            # under body = maximum weight and grip for backward push.
            # This forces IK to extend the tibia more, pressing into ground.
            dz = -10.0 * math.sin(math.pi * t)
        else:
    ...
    def compute_body_shift(self, phase: float, speed: float,
        ...
        Returns (shift_x, shift_y) to add to ALL foot positions.
        Shifting feet in one direction moves the body the opposite way.
        """
        if abs(speed) < 0.01:
            return (0.0, 0.0)
        ...
        # Find which leg(s) are currently in swing and compute support centroid
        swing_weight = {}  # how much each leg is "in swing" (0-1)
        for leg_id, leg_offset in phase_offsets.items():
            leg_phase = (phase + leg_offset) % 1.0
            if leg_phase >= duty:
                # In swing - weight = how deep into swing (peak at middle)
                t = (leg_phase - duty) / (1.0 - duty)
                swing_weight[leg_id] = math.sin(math.pi * t)
            else:
                swing_weight[leg_id] = 0.0

        # Compute weighted support centroid (legs NOT in swing)
        total_support = 0.0
        cx, cy = 0.0, 0.0
        ...
        # Shift body toward support centroid by shifting feet opposite
        # Scale factor: 0.12 gives ~10mm shift, enough for weight transfer
    ...
    def set_gait(self, gait: GaitType):
        with self._lock:
            ...
            if gait == GaitType.WALK:
                ...
                self.gait_cfg.step_length = 70.0
            else:
