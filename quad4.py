...
Version: 0.22
Date:    2026-02-15
Changes: v0.22 - Added deep servo diagnostic (press 'r'). Resets
         angle limits to joint mode, toggles torque, and attempts
         large position moves to diagnose unresponsive servo.
         v0.21 - Added RR shoulder test command (press 't' during
         keyboard control). Sweeps servo ID 2 in/out to diagnose
         responsiveness.
         v0.20 - Restored identify_all_servos method that was lost
         during earlier edits.
         v0.19 - Fixed RR shoulder drift. Removed +5deg offsets from
         FR and RR shoulders. Set shoulder servos to speed=300 for
         better position holding under load. Femur/tibia stay at
         speed=0 (max) for fast response.
         v0.18 - Lock coxa/shoulder angle to standing value during
         walking. Prevents shoulder drift caused by foot Z changes
         affecting IK coxa calculation. Coxa raw position now sent
         every cycle at the fixed standing angle.
         v0.17 - MAJOR fix: Set servo speed to 0 (max/unlimited).
         Previous speed=250 meant servos couldnt keep up with 50Hz
         position commands. Now servos chase target as fast as possible.
         Cycle time 1.2->2.0s for slow deliberate steps.
         Step length 50->40mm.
         v0.16 - MAJOR gait fix. Body height 120->130mm giving 33mm
         max foot lift headroom. Step height 20->30mm. Replaced raised
         cosine trajectory with half-sine for sharp foot lift (3x
         faster ground clearance at swing start). Feet were dragging
         on carpet because lift was too gradual with old profile.
         v0.15 - Fixed gait parameters. Step height 35->20mm (was
         exceeding AX-12A 150deg limit during swing causing servo
         clipping = feet couldnt complete swing = no forward motion).
         Step length 45->60mm, cycle time 1.0->1.2s.
         v0.14 - Added +5deg shoulder offset on FR and RR to tuck
         right-side legs slightly more under body.
         v0.13 - Reduced foot lateral from 12mm to 5mm. Legs now only
         5deg from vertical so feet sit flat on ground.
         v0.12 - Flipped tibia inversion on FL (True->False) and RL
         (False->True) so all tibias point downward.
         v0.11 - Flipped femur inversion on FL (True->False) and RL
         (False->True) so all femurs point inward toward body center.
         v0.10 - Flipped FL shoulder and FR shoulder inversions. Front
         legs now: FL all inverted, FR shoulder not inverted + femur/
         tibia inverted. Rear legs unchanged (working correctly).
         v0.9 - Flipped femur/tibia inversion on both front legs (were
         pointing up). Reduced foot lateral to 12mm to tuck legs more
         under body.
         v0.8 - Fixed foot lateral (20mm) and body height (120mm) for
         proper cat crouch. Coxa now tilts ~70deg (20deg from vertical).
         Fixed FR/RR inversions (only coxa inverted, not femur/tibia).
         v0.7 - Rewrote IK for cat-style geometry (coxa=sideways,
         femur/tibia=fore-aft sagittal plane). Fixed foot lateral
         position to not exceed coxa length. All inversions reset.
         v0.6 - Added coxa offset hack (removed in v0.7)
         v0.5 - Fixed unicode characters for Pi terminal
         v0.4 - Swapped RL/RR leg servo IDs
         v0.3 - Increased identify flashes to 10
         v0.2 - Added actual servo IDs and measurements
...
class GaitConfig:
    ...
    step_height: float = 30.0      # Foot lift height mm
    step_length: float = 40.0      # Forward travel per step mm (conservative)
    cycle_time: float = 2.0        # Full gait cycle seconds (slow and deliberate)
    duty_factor: float = 0.5       # 0.5=trot, 0.75=walk
    body_height: float = 130.0     # Standing height mm (taller = more foot lift room)
...
LEG_SERVO_CONFIG: Dict[LegID, List[ServoJointConfig]] = {
    ...
    LegID.FL: [
        ...
        ServoJointConfig(servo_id=10, offset_deg=0.0, inverted=False),   # FL Femur   FLIPPED
    ...
    LegID.RL: [
        ...
        ServoJointConfig(servo_id=6,  offset_deg=0.0, inverted=True),    # RL Leg     FLIPPED
    ],
    LegID.RR: [
        ServoJointConfig(servo_id=2,  offset_deg=0.0,  inverted=True),    # RR Shoulder
        ServoJointConfig(servo_id=1,  offset_deg=0.0, inverted=False),   # RR Femur
        ServoJointConfig(servo_id=4,  offset_deg=0.0, inverted=False),   # RR Leg
    ],
}
...
class QuadrupedWalker:
    ...
    def __init__(self, leg_dims: LegDimensions = None,
        ...
        # Since femur/tibia only swing fore/aft, max lateral = coxa length.
        # Small lateral value = coxa tilts steeply down = cat-like stance.
        # lateral=20mm gives ~70deg coxa (20deg from vertical) - good cat crouch.
        L1 = self.leg_dims.coxa_length
        foot_lateral = 5.0   # Nearly vertical legs, feet flat on ground
...
def run_keyboard_control(walker: QuadrupedWalker):
    ...
    print("  r       = deep diagnose RR shoulder (reset modes)")
    print("  q       = quit")
    print("=" * 50 + "\n")
